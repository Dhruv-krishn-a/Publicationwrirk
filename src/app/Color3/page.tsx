...
function SectionTitle({ eyebrow, title, subtitle }: { eyebrow: string; title: string; subtitle: string }) {
  return (
    <div className="max-w-3xl">
      <div className="mb-3 text-[11px] uppercase tracking-[0.4em] text-emerald-200/70">{eyebrow}</div>
...
function Pill({ children }: { children: ReactNode }) {
  return (
    <span className="inline-flex items-center rounded-full border border-emerald-400/25 bg-emerald-400/10 px-3 py-1 text-[11px] tracking-[0.28em] uppercase text-emerald-100/90 shadow-[0_1px_0_rgba(255,255,255,0.04)] backdrop-blur-sm">
      {children}
    </span>
  );
}
...
function ServiceLine({ title, text }: { title: string; text: string }) {
  return (
    <div className="group rounded-[28px] border border-white/10 bg-white/5 p-5 md:p-6 shadow-[0_24px_80px_rgba(0,0,0,0.22)] transition-all duration-300 hover:-translate-y-1 hover:border-emerald-400/30 hover:bg-white/8 hover:shadow-[0_30px_120px_rgba(0,0,0,0.32)]">
...
function FloatingActionButton({ href, icon: Icon, label, className }: { href: string; icon: LucideIcon; label: string; className?: string }) {
  return (
    <a
      href={href}
      aria-label={label}
      className={`group flex items-center overflow-hidden rounded-full border border-white/10 bg-[#0c1418]/90 shadow-[0_20px_60px_rgba(0,0,0,0.35)] backdrop-blur-md transition-all duration-300 hover:scale-[1.03] hover:border-emerald-400/35 ${className ?? ''}`}
    >
      <span className="flex h-14 w-14 items-center justify-center rounded-full bg-emerald-500/15 text-emerald-200 transition-all duration-300 group-hover:bg-emerald-500/25">
...
function QuoteMark() {
  return (
    <div className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/5 text-2xl leading-none text-emerald-200">
      “
    </div>
  );
}
...
export default function WrirkLandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#1B0B2E] text-slate-100">
      <div className="pointer-events-none fixed inset-0 opacity-[0.28] [background-image:radial-gradient(rgba(52,211,153,0.12)_1px,transparent_1px)] [background-size:24px_24px]" />
      <div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_top_right,rgba(52,211,153,0.16),transparent_28%),radial-gradient(circle_at_bottom_left,rgba(4,120,87,0.14),transparent_30%)]" />
      ...
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#1B0B2E]/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 md:px-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[11px] tracking-[0.32em] uppercase text-emerald-100/65">Research Publication Support</div>
            </div>
          </div>

          <nav className="hidden items-center gap-7 text-[11px] uppercase tracking-[0.28em] text-slate-300 md:flex">
            <a href="#services" className="transition hover:text-emerald-200">Services</a>
            <a href="#profile" className="transition hover:text-emerald-200">Profile</a>
            <a href="#journal" className="transition hover:text-emerald-200">Journal Match</a>
            <a href="#process" className="transition hover:text-emerald-200">Process</a>
            <a href="#contact" className="transition hover:text-emerald-200">Contact</a>
          </nav>

          <a
            href="#contact"
            className="group inline-flex items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-400/10 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.26em] text-emerald-100 transition hover:border-emerald-300/40 hover:bg-emerald-400/15 hover:text-white"
          >
      ...
      <main className="relative mx-auto max-w-7xl px-5 md:px-8">
        ...
        <section className="grid gap-12 py-14 md:grid-cols-[1.12fr_0.88fr] md:gap-8 md:py-20">
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                href="#contact"
                className="group inline-flex items-center gap-2 rounded-full bg-emerald-400 px-6 py-3 text-xs font-semibold uppercase tracking-[0.28em] text-slate-950 shadow-[0_18px_50px_rgba(52,211,153,0.24)] transition hover:-translate-y-0.5 hover:bg-emerald-300"
              >
                Book a free consultation <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
              </a>
              <a
                href="#services"
                className="group inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/5 px-6 py-3 text-xs font-semibold uppercase tracking-[0.28em] text-slate-100 transition hover:-translate-y-0.5 hover:border-emerald-300/30 hover:bg-white/8"
              >
            ...
            <div className="mt-10 grid grid-cols-2 gap-3 md:grid-cols-4">
              {stats.map((stat) => (
                <div key={stat.label} className="rounded-[24px] border border-white/10 bg-white/5 p-4 shadow-[0_18px_60px_rgba(0,0,0,0.18)] transition hover:-translate-y-1 hover:border-emerald-400/20 hover:bg-white/7">
            ...
            <div className="mt-8 rounded-[30px] border border-white/10 bg-white/5 p-5 md:p-6">
              <div className="flex flex-wrap gap-2">
                {reasons.map((item) => (
                  <span key={item} className="rounded-full border border-white/10 bg-[#0c171b] px-3 py-2 text-[11px] uppercase tracking-[0.22em] text-slate-200 transition hover:border-emerald-300/30 hover:bg-white/8">
                    {item}
                  </span>
                ))}
              </div>
          ...
          <div className="relative flex items-center justify-center">
            <div className="absolute right-6 top-8 h-72 w-72 rounded-full bg-emerald-400/10 blur-3xl" />
                ...
                <div className="aspect-[4/5] p-5 md:p-7">
                  ...
                  <div className="mt-6 rounded-[26px] border border-white/10 bg-[#0c171b]/90 p-4 shadow-sm md:p-6">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="text-[11px] uppercase tracking-[0.25em] text-emerald-200/80">Journal shortlist</div>
                        <div className="mt-2 text-2xl font-semibold tracking-tight text-slate-50">Best-fit journal match</div>
                      </div>
                      <div className="rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1 text-[11px] uppercase tracking-[0.25em] text-emerald-100">
                        94% fit
                      </div>
                    </div>
        ...
        <section className="py-16 md:py-20">
          <div className="grid gap-6 rounded-[36px] border border-white/10 bg-white/5 p-7 md:grid-cols-[1fr_0.9fr] md:p-10">
            <div>
              <div className="text-[11px] uppercase tracking-[0.35em] text-emerald-200/70">SEO focus</div>
            ...
            <div className="rounded-[30px] border border-emerald-400/15 bg-[#0c171b] p-6 shadow-[0_20px_80px_rgba(0,0,0,0.28)]">
            ...
            {painPoints.map((item) => (
              <div key={item.title} className="rounded-[28px] border border-white/10 bg-white/5 p-6 transition hover:-translate-y-1 hover:bg-white/8">
                <div className="flex items-center justify-between">
                  <div className="h-10 w-10 rounded-full border border-emerald-400/20 bg-emerald-400/10" />
        ...
        <section id="profile" className="py-20 md:py-28">
          ...
          <div className="mt-14 grid gap-5 md:grid-cols-5">
            {profileSteps.map((step, index) => (
              <div key={step.title} className="group relative rounded-[28px] border border-white/10 bg-white/5 p-5 md:p-6 shadow-[0_20px_80px_rgba(0,0,0,0.2)] transition-all duration-300 hover:-translate-y-1 hover:border-emerald-400/25 hover:bg-white/7">
                <div className="flex items-center justify-between">
                  <div className="text-[11px] uppercase tracking-[0.3em] text-slate-400">0{index + 1}</div>
                  {index < profileSteps.length - 1 ? <ArrowRight className="hidden h-4 w-4 text-emerald-300 md:block transition-transform group-hover:translate-x-1" /> : null}
                </div>
        ...
        <section className="grid gap-8 border-y border-white/10 py-16 md:grid-cols-[1.05fr_0.95fr] md:py-20">
          <div>
            <div className="text-[11px] uppercase tracking-[0.35em] text-emerald-200/70">Why publications matter</div>
            <div className="mt-4 max-w-2xl text-5xl font-semibold leading-[0.98] tracking-tight text-slate-50 md:text-7xl">
              ...
              <span className="block text-emerald-300 italic">It is the beginning of academic influence.</span>
            </div>
          ...
          <div className="rounded-[34px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.08),rgba(255,255,255,0.04))] p-8 shadow-[0_20px_80px_rgba(0,0,0,0.24)] md:p-10">
            ...
            <p className="mt-6 text-2xl md:text-4xl leading-tight italic text-emerald-100">
        ...
        <section id="journal" className="py-20 md:py-28">
          ...
          <div className="mt-14 grid gap-8 lg:grid-cols-[0.96fr_1.04fr]">
            <div className="rounded-[34px] border border-white/10 bg-white/5 p-6 md:p-8">
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {journalFactors.map((factor, idx) => (
                  <span key={factor} className={`rounded-full border px-4 py-2 text-sm transition ${idx === 0 ? 'border-emerald-300/25 bg-emerald-400/10 text-emerald-100' : 'border-white/10 bg-white/5 text-slate-300 hover:bg-white/8'}`}>
                    {factor}
                  </span>
                ))}
              </div>
              ...
              <div className="mt-8 space-y-4">
                <div className="rounded-[24px] border border-white/10 bg-[#0c171b] p-5">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1 text-[11px] uppercase tracking-[0.25em] text-emerald-100 shadow-sm">
                      High fit
                    </div>
                  </div>
                </div>
            ...
            <div className="rounded-[34px] border border-white/10 bg-white/5 p-6 md:p-8">
              ...
              <div className="mt-6 rounded-[28px] border border-emerald-400/15 bg-[linear-gradient(135deg,rgba(52,211,153,0.18),rgba(4,120,87,0.18))] p-6 text-white shadow-[0_25px_80px_rgba(0,0,0,0.24)]">
        ...
        <section className="py-16 md:py-20">
          <div className="grid gap-6 rounded-[36px] border border-white/10 bg-white/5 p-7 md:grid-cols-[1fr_1fr] md:p-10">
            <div>
              <div className="text-[11px] uppercase tracking-[0.35em] text-emerald-200/70">Who we help</div>
              <h2 className="mt-4 text-3xl md:text-5xl font-semibold leading-[1.02] tracking-tight text-slate-50">
        ...
        <section className="grid gap-8 border-y border-white/10 py-16 md:grid-cols-[0.95fr_1.05fr] md:py-20">
          <div>
            <div className="text-[11px] uppercase tracking-[0.35em] text-emerald-200/70">Research career tips</div>
            ...
            <div className="mt-8 rounded-[30px] border border-white/10 bg-white/5 p-6 md:p-8">
              ...
              <p className="mt-4 text-2xl md:text-3xl italic leading-tight text-emerald-100">
                ...
                >
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 text-lg font-semibold text-slate-50">
                      <CircleHelp className="h-5 w-5 text-emerald-300" />
                      <span>{faq.q}</span>
                    </div>
        ...
        <section id="contact" className="pb-20 md:pb-28">
          <div className="grid overflow-hidden rounded-[40px] border border-white/10 bg-white/5 shadow-[0_30px_120px_rgba(0,0,0,0.25)] lg:grid-cols-[0.95fr_1.05fr]">
            <div className="bg-[linear-gradient(180deg,rgba(52,211,153,0.18),rgba(4,120,87,0.22))] p-8 text-white md:p-12">
            ...
            <div className="p-8 md:p-12">
              <div className="text-[11px] uppercase tracking-[0.35em] text-emerald-200/70">Request an assessment</div>
              ...
              <form className="mt-8 grid gap-4 md:grid-cols-2">
                {['Full name', 'Email address', 'Phone number', 'Research area'].map((label, idx) => (
                  <label key={label} className={idx === 3 ? 'md:col-span-2' : ''}>
                    ...
                    <input
                      ...
                      className="w-full rounded-[18px] border border-white/10 bg-[#0c171b] px-4 py-3 text-sm text-slate-100 outline-none transition placeholder:text-slate-500 focus:border-emerald-300/40 focus:bg-[#0e1b20]"
                    />
                  </label>
                ))}
                ...
                <button
                  type="button"
                  className="group inline-flex items-center justify-center gap-2 rounded-full bg-emerald-400 px-6 py-3 text-xs font-semibold uppercase tracking-[0.26em] text-slate-950 transition hover:-translate-y-0.5 hover:bg-emerald-300 md:col-span-2"
                >
                  Book a free consultation <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
                </button>
              </form>
